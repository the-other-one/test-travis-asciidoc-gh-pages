...
- link:javadoc/org/beryx/textio/InputReader#withInputMasking-boolean-.html[input masking] - useful when reading sensitive data such as passwords.
- link:javadoc/org/beryx/textio/InputReader#with.html[defaultValue] - the value to be used if the user pressed Enter.
- link:javadoc/org/beryx/textio/InputReader#with.html[possible values] - necessary if the value to be read must be chosen from a list of allowed values.
- link:javadoc/org/beryx/textio/InputReader#with.html[error messages] - used to provide custom error messages.
- link:javadoc/org/beryx/textio/InputReader#with.html[property name] - if configured, the name of the property to be read will appear in error messages.
...
link:javadoc/org/beryx/textio/InputReader.html#read-java.lang.String...-[ read(String... prompt)]
or
